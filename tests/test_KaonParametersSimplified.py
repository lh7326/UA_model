...
from model_parameters.KaonParametersSimplified import KaonParametersSimplified, Parameter


class TestModelParametersSimplified(TestCase):

    def setUp(self):
        self.parameters = KaonParametersSimplified(
            0.5, 0.6, 0.7, 0.8,
            0.1,  0.78266, 0.00868,
            0.2, 1.410, 0.29,
            0.15, 1.019461, 0.315,
    ...
    def test___setitem__(self):
        ...
        with self.subTest(msg='Testing parameter name checking'):
            with self.assertRaises(ValueError):
                self.parameters['a_phi'] = Parameter(name='new name', value=2.0, is_fixed=True)

        with self.subTest(msg='Testing a successful call'):
            self.parameters['a_phi'] = Parameter(name='a_phi', value=95.412, is_fixed=True)
            self.assertEqual(self.parameters['a_phi'].value, 95.412)
    ...
    def test_release_parameters(self):
        self.parameters.fix_parameters(['a_rho_prime'])
        self.parameters.release_parameters(['a_rho_prime'])
        self.assertFalse(self.parameters['a_rho_prime'].is_fixed)

    def test_release_all_parameters(self):
        self.parameters.fix_parameters(['a_phi', 'mass_rho_prime', 't_in_isovector'])
        self.parameters.release_all_parameters()
    ...
    def test_fix_all_parameters(self):
        self.parameters.release_parameters(['a_phi', 'mass_rho_prime', 't_in_isovector'])
    ...
    def test_get_fixed_values(self):
        ...
        self.assertListEqual(
            ...
            [0.5, 0.6, 0.8, 0.78266, 0.00868, 1.410, 0.775, 0.1474, 1.465],
        )
    ...
    def test_get_free_values(self):
        ...
        self.assertListEqual(
            ...
            [0.7, 0.8, 0.1, 0.2, 0.35, 0.159, 1.465, 0.4, 1.57, 0.2],
        )
    ...
    def test_get_all_values(self):
        self.assertListEqual(
            ...
            [0.5, 0.6, 0.7, 0.8, 0.1,  0.78266, 0.00868,
             0.2, 1.410, 0.29, 0.15, 1.019461, 0.315,
             0.3, 1.67, 0.004249, 2.159, 0.137,
             0.35, 0.775, 0.1474, 0.159, 1.465, 0.4, 1.57, 0.2],
        )
    ...
    def test_from_list_to_list_consistency(self):
        ...
        model_parameters = KaonParametersSimplified.from_list(list_of_parameters)
        self.assertIsInstance(model_parameters, KaonParametersSimplified)
        self.assertListEqual(list_of_parameters, model_parameters.to_list())

    def test_to_list_from_list_consistency(self):
        recreated = KaonParametersSimplified.from_list(self.parameters.to_list())
        self.assertEqual(
